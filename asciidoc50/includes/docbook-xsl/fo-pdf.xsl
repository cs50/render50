...
<xsl:stylesheet version="1.0"
  ...
  <!-- DJM -->
  <!-- discourage page breaks within source code -->
  ...
  <!-- DJM -->
  <!-- ?cs50-hacker? is outputted by bin/asciidoctor if Asciidoc input has :hacker: attribute -->
  <xsl:param name="draft.mode">
    <xsl:if test="/processing-instruction('cs50-hacker')">yes</xsl:if>
  </xsl:param>

  <!-- piggyback off of $admon.graphics.path, which is passed in as a parameter via lib/asciidoctor-fopub-master/fopub -->
  <xsl:param name="draft.watermark.image">
    <xsl:if test="/processing-instruction('cs50-hacker')">
      <xsl:value-of select="$admon.graphics.path"/>../../../../../../includes/hacker.png
    </xsl:if>
  </xsl:param>

  <!-- DJM -->
  <!-- remove "Draft" from atop pages -->
  <!-- lib/asciidoctor-fopub-master/build/fopub/docbook/fo/pagesetup.xsl -->
  ...
  <xsl:template match="db:phrase">
    <fo:inline>
      ...
      <xsl:if test="@role='line-through' or @role='overline' or @role='underline'">
      ...
      <xsl:choose>
        ...
        <xsl:when test="@role='yellow'">
          <xsl:attribute name="color">#cccc00</xsl:attribute>
        </xsl:when>
      </xsl:choose>
  ...
  <!-- DJM -->
  <xsl:param name="paper.type">USletter</xsl:param> <!-- alternative size is USletter -->
  ...
  <xsl:template name="toc.line">
    ...
    <!-- DJM: hide preamble from TOC -->
    <xsl:if test="$id != 'N1000F'">
